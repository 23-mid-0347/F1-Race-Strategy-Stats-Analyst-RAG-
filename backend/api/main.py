...
def constructor_standings(
    season: Optional[int] = Query(
        None,
        description="Filter to one season, e.g. 2023. Omit for all seasons combined.",
    ),
    ...
    rows = db.execute(
        text("""
            ...
            WHERE (
                ...
                OR se.year = CAST(:season AS INTEGER)
            )
            GROUP BY c.name
...
@app.get("/constructors/count")
def constructor_count(
    season: Optional[int] = Query(
        None,
        description="Filter to one season, e.g. 2023.",
    ),
    db: Session = Depends(get_db),
):
    count = db.execute(
        text("""
            SELECT COUNT(DISTINCT res.constructor_id)
            FROM results res
            JOIN sessions s ON s.session_id = res.session_id
            JOIN events e ON e.event_id = s.event_id
            JOIN seasons se ON se.season_id = e.season_id
...
def driver_count(
    ...
    count = db.execute(
        text("""
            SELECT COUNT(DISTINCT res.driver_id)
            FROM results res
            JOIN sessions s ON s.session_id = res.session_id
            JOIN events e ON e.event_id = s.event_id
            JOIN seasons se ON se.season_id = e.season_id
...
def driver_podiums(
    ...
    rows = db.execute(
        text("""
            ...
            JOIN events e ON e.event_id = s.event_id
            JOIN seasons se ON se.season_id = e.season_id
            WHERE res.finish_position BETWEEN 1 AND 3
              AND s.session_type = 'Race'
              AND (
                  CAST(:season AS INTEGER) IS NULL
                  OR se.year = CAST(:season AS INTEGER)
...
def fastest_lap(
    ...
    if row is None:
        raise HTTPException(
            status_code=404,
            detail=f"No lap data found for session_id {session_id}",
        )
...
def get_driver(
    ...
    if row is None:
        raise HTTPException(
            status_code=404,
            detail=f"No driver found with driver_id {driver_id}",
        )
...
def get_driver_stats(
    ...
    season: Optional[int] = Query(
        None,
        description="Filter to one season, e.g. 2023. Omit for career totals.",
    ),
    db: Session = Depends(get_db),
):
    driver = db.execute(
    ...
    row = db.execute(
        text("""
            SELECT
                ...
                COUNT(*) FILTER (
                    WHERE res.status NOT IN ('Finished')
                    AND res.status NOT LIKE '%Lap%'
                    AND s.session_type = 'Race'
                ) AS dnfs,

                ROUND(
                    AVG(res.finish_position)
                    FILTER (WHERE s.session_type = 'Race'),
                    2
                ) AS avg_finish
...
def list_events(
    season: Optional[int] = Query(
        None,
        description="Filter to one season, e.g. 2023. Omit for all seasons.",
    ),
    limit: int = Query(
        20,
        ge=1,
...
def get_event_results(
    ...
    if event is None:
        raise HTTPException(
            status_code=404,
            detail=f"No event found with event_id {event_id}",
        )
